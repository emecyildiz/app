  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="w-full max-w-md text-center glass rounded-2xl p-8">
        ...
        <h1 className="text-2xl font-bold text-white mb-2">E‑posta Değişikliği</h1>
        <p className="text-gray-400">{status === 'loading' ? 'İşlem sürüyor...' : status === 'success' ? 'E‑posta güncellendi!' : 'Bağlantı geçersiz.'}</p>
      </motion.div>
    </div>
  )
}

    ...
    setTimeout(function() {
        ...
        navLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                // Sayfa yükleme - tüm sayfalar için
                const page = this.dataset.page;
                if (page) {
                    // CinemaHubApp varsa onu kullan
                    if (window.app && window.app instanceof CinemaHubApp) {
                        window.app.loadPage(page);
                    } else {
                        // Fallback olarak man.js'deki loadPage'i kullan
                ...
                // Dinamik Filmler sayfası yükleme (eski kod - geriye uyumluluk için)
                    ...
                    if (content) {
                        ...
                        fetch('pages/Filmler.html')
                            ...
                            .then(html => {
                                ...
                                const extractedContent = bodyMatch ? bodyMatch[1] : html;
                                
                                // Fix CSS paths
                                const fixedContent = fixCSSPaths(extractedContent);
                                content.innerHTML = fixedContent;

-- Supabase RLS (Row Level Security) Düzeltme Scripti - Güncellenmiş Versiyon
-- Bu script Supabase'in standart tablo yapısına uygun olarak hazırlanmıştır
        ...
        CREATE POLICY "Movies can be created by admin and operator" ON public.movies
            FOR INSERT WITH CHECK (
                EXISTS (
                    ...
                    AND (auth.users.raw_user_meta_data->>'role' = 'ADMIN' OR auth.users.raw_user_meta_data->>'role' = 'OPERATOR')
                )
            );
